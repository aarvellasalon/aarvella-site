...
4. **`full_name` defaulting to the customer's email address** on first login — Auth0 defaults its `name` claim to the email itself when no display name is collected, and `register.php` never collects one (signup is 100% delegated to Auth0 Universal Login, no custom form). `sync-customer.php`'s existing empty-name fallback never caught this since `name` wasn't empty, just uselessly equal to the email. Fixed in `sync-customer.php` (`6b2db96`) by treating an email-equal name the same as a missing one, falling through to the existing local-part-of-email fallback. A scoped one-off `UPDATE` was also provided (not run) to backfill existing rows where `full_name = email`.

Items 1-4 are committed, pushed, and confirmed deployed (a GitHub webhook auto-deploys to cPanel on push to `origin/main` — see "Hosting and deployment"), along with the two "Current priorities" items below (booking-popup click-through, portal shell rendering after the query rewrite).
...
### Legacy endpoint removal and config centralization — 2026-07-29

**Status: done, committed, pushed, and deployed.**

1. **Unused legacy API endpoints removed** (`9e8b13c`) — investigation found 5 orphaned files, not the 3 originally named, all confirmed zero-referenced anywhere in the frontend: `api/appointments/create.php`, `api/services/list.php`, `api/stylists/list.php` (superseded by the CRM integration), `api/contact/submit.php` (no contact form UI was ever built to call it — find-us.html's "contact form" is a demo stub with `onsubmit="alert(...)"`, doesn't submit anywhere), and `api/customer/me.php` (completely empty file). Shared config (`api/config/{cors,db,database,helpers}.php`) kept — `newsletter/subscribe.php` still depends on it.
2. **WhatsApp number centralized** (`a7c8311`) — was hardcoded independently in 8 files. New `assets/js/site-config.js` (sets `window.AARVELLA_CONFIG`, loaded before `booking.js` on all 24 pages that use it) and `account/site-config.php` (`AARVELLA_WHATSAPP_NUMBER`, zero dependencies so it's safe from `account-error.php`) are now the two canonical sources; `booking.js`, `ai-chat.js`, and the 4 `account/*.php` portal pages all reference them instead of a literal.
3. **A real inconsistency found and fixed**: `find-us.html` said "Opening hours to be finalized" while the shared footer partial (loaded on every other page) already stated specific hours — two pages on the live site contradicted each other. Footer's hours confirmed correct; `find-us.html` updated to match (`6f68ea7`).
4. **Address/hours/geo-coordinates centralization was scoped out deliberately** — investigation found the shared `footer.html` partial (already loaded via the existing runtime-JS-partial pattern on ~23 of 27 top-level pages) is already the de facto single source for most pages. Remaining duplication is narrower than originally assumed, concentrated in: the 4 blog articles' bespoke footers (already separately tracked as priority item 7), and JSON-LD `LocalBusiness` schema blocks on `makeup.html`/`hair.html`/2 blog articles. True de-duplication of these would need a build-time include/templating step — new tooling, not something to introduce without explicit approval given this is a static-HTML site by design. Left as manually-synced for now.

### Design token consolidation and index.html CSS extraction — 2026-07-29

**Status: done, committed, pushed, and deployed.**

Audited all 19 files across the repo that define a `:root` block before touching anything, since CLAUDE.md explicitly protects the black/gold/white/frosted-glass design system. Found real color drift, not just copy-paste duplication — e.g. three different `--charcoal` values (`#141414`, `#151515`, `#1A1A1A`) and `navigation.css`'s gold (`#d7b65d`) being a distinctly brighter shade than the core gold (`#d4af37`), plausibly intentional (nav floats over both dark and light page backgrounds). Given that, explicitly chose an "organize only, preserve every value" approach over reconciling drifted values to one canonical color — no page's rendered appearance changed as a result of this work.

1. **New `assets/css/tokens.css`** — the core palette (`--gold`, `--dark-gold`, `--black`, `--charcoal`, `--white`, `--muted`, `--line`, `--glass`, `--ease-out`, `--transition`) that was duplicated near-identically across 5 pages' inline `<style>` blocks. 4 of those 5 (`bridal.html`, `privacy.html`, `careers.html`, `terms.html`) had byte-identical blocks — their local `:root` was deleted outright in favor of linking the shared file. `index.html` differed on one value (`--charcoal: #1A1A1A` vs. the other 4's `#151515`) plus had 3 extra properties of its own (`--ease-soft`, `--duration-fast`, `--duration-medium`) — kept as a small local override block after linking `tokens.css`, so its computed values are unchanged.
2. **`index.html`'s ~1,000-line inline `<style>` block extracted verbatim into `assets/css/index.css`** — zero rule changes, just moved out of the page (plus the small token override above). Only one `url()` reference existed in the whole block and it was an absolute external URL, so no relative-path concerns. Verified via local `php -S`: `index.html` and both new CSS files return 200; diff confirmed the change is fully localized (no accidental line-ending rewrite of the rest of the file).
3. **Deliberately left untouched**: component-level token systems (`buttons.css`, `navigation.css`, `customer-portal.css`, `customer-portal-pages.css`, `service-showcase.css`, `article.css`) — each already uses its own uniquely namespaced variables (`--av-btn-*`, `--av-nav-*`, `--portal-*`, `--showcase-*`), so they aren't part of the duplication problem this priority item was about. `account-error.php`/`verify-email.php` also share an identical but deliberately separate minimal dark-mode palette — not merged into the marketing-page core since it's a different design context (bare-bones fallback pages). `aarvella-pages.css` and `blog.css` being byte-identical CSS files (loaded by non-overlapping page sets: `find-us.html`/`stylists.html`/`about.html` vs. `blog.html`/blog articles) is a separate, smaller finding, not yet acted on — worth a follow-up pass if this area gets revisited.
...
3. ~~Decide the fate of the unused legacy booking endpoints~~ **Done, 2026-07-29** — deleted (`9e8b13c`). See new "Legacy endpoint removal and config centralization" section under "Active work" above.
4. ~~Centralize hardcoded config (WhatsApp number, salon address, business hours, geo-coordinates)~~ **Partially done, 2026-07-29** — WhatsApp number centralized into shared JS/PHP config (`a7c8311`); a real address/hours inconsistency found and fixed (`6f68ea7`). Address/hours/geo still duplicated in static HTML/JSON-LD — see the new section for why that part is out of scope without new build tooling.
5. ~~Consolidate CSS design tokens into one shared source~~ **Partially done, 2026-07-29** — see new "Design token consolidation and index.html CSS extraction" section under "Active work" above. New `assets/css/tokens.css` covers the core palette that was duplicated across 5 pages' inline blocks (all now byte-identical or safely reconciled, zero visual change). Component-level token systems (buttons.css, navigation.css, customer-portal.css, service-showcase.css, article.css) were deliberately left untouched — already uniquely namespaced, not part of this duplication problem. `aarvella-pages.css`/`blog.css` being identical to each other is a separate, smaller finding not yet acted on.
6. ~~Move `index.html`'s remaining ~1,000-line inline `<style>` block into `assets/css/`~~ **Done, 2026-07-29** — extracted verbatim into `assets/css/index.css`, zero rule changes. See same new section.
